...
from .ElectronTemperature import Temperature
...
class Ionosphere(object):
    ...
    def read_config(self, config_file):
        ...
        start_utime = (dt.datetime.fromisoformat(config['GENERAL']['STARTTIME'])-dt.datetime.utcfromtimestamp(0)).total_seconds()
        # starttime = dt.datetime.fromisoformat(config['GENERAL']['STARTTIME'])

        self.density = Density(start_utime, config['DENSITY'])
        self.velocity = Velocity(start_utime, config['VELOCITY'])
