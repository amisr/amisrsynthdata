...
class Radar(object):
    ...
    def __init__(self, config):
        ...
        self.acf_slant_range, self.acf_lat, self.acf_lon, self.acf_alt = self.calculate_acf_gates(slant_range_config)
        self.slant_range, self.lat, self.lon, self.alt = self.calculate_gates(alt_bin_config)
    ...
    def calculate_acf_gates(self, slant_range_config):
        ...
        lat_p, lon_p, alt_p = pm.aer2geodetic(self.beam_azimuth[:,None], self.beam_elevation[:,None], slant_range_p[None,:], self.site_lat, self.site_lon, self.site_alt)
    ...
    def calculate_gates(self, alt_bins_config):
        ...
        with warnings.catch_warnings():
            ...
            slant_range = np.array([[np.nanmean(np.where((beam>=altbins[i]) & (beam<altbins[i+1]), self.acf_slant_range, np.nan)) for i in range(len(altbins)-1)] for beam in self.acf_alt])
    ...
    def kvec_all_gates(self):
        ...
        ke_all, kn_all, ku_all = pm.uvw2enu(kx[:,None], ky[:,None], kz[:,None], self.lat, self.lon)
