...
class SyntheticData(object):
    ...
    def __init__(self, config):
        ...
        output_filename = config['GENERAL']['output_filename']  # input directly to write function
        err_coef = config['GENERAL']['err_coef']
        add_noise = config['GENERAL']['noise']  # option on calc_radar_measurements
        ...
        self.ne_err, self.ti_err, self.te_err, self.vlos_err, self.ne_notr_err = self.generate_errors(err_coef)


#    def run_everything()
#        # Can be in init??
#        create_measurement_arrays()
#        add_nois()  # option
...
#            self.summary_plot(config['SUMMARY_PLOT'])
    ...
    def generate_time_array(self, starttime, endtime):
        ...
        utime = np.array([ust+np.arange(0,num_tstep)*self.radar.integration_period, ust+np.arange(1,num_tstep+1)*self.radar.integration_period]).T
        time = np.array([[dt.datetime.utcfromtimestamp(t) for t in ut] for ut in utime])
        return utime, time

    def generate_radar_measurements(self):
        # caculate scalar ionosphere parameters at each fitted radar bin
        ne = self.iono.density(self.utime[:,0], self.radar.lat, self.radar.lon, self.radar.alt)
        ti = self.iono.itemp(self.utime[:,0], self.radar.lat, self.radar.lon, self.radar.alt)
        te = self.iono.etemp(self.utime[:,0], self.radar.lat, self.radar.lon, self.radar.alt)
        ...
        ne_notr = self.iono.density(self.utime[:,0], self.radar.acf_lat, self.radar.acf_lon, self.radar.acf_alt)
...
#        # Everything below this can be a seperate "create time arrays" function - only useful for output
#
...
#
#        ne_notr = self.iono.density(self.utime[:,0], self.radar.acf_lat, self.radar.acf_lon, self.radar.acf_alt)
#        return ne, te, ti, vlos, ne_notr

#    def output_fitted_params(self):
#
...
#        ne_notr_err = err_coef[0] * self.radar.acf_slant_range**2
#
#        return ne_err, ve_err, te_err, ti_err, ne_notr_err

#    def calc_errors(self, error_coeff):
...
#        return ne, ti, te, vlos, ne_notr
    ...
    def generate_beamcodes(self):
        ...
        beamcodes = np.array([self.radar.beam_codes, self.radar.beam_azimuth, self.radar.beam_elevation,  self.radar.beam_ksys]).T
        return beamcodes

    def generate_fitted_params(self):

        FittedParams = {'Altitude':self.radar.alt, 'IonMass':self.iono.ion_mass, 'Range':self.radar.slant_range}
        ...
        # fit info
        FitInfo = {'chi2':np.full(s, 1.0), 'dof':np.full(s, 26), 'fitcode':np.full(s, 1), 'nfev':np.full(s, 0)}


        # calculate density in ACF bins
        NeFromPower = {'Altitude':self.radar.acf_alt, 'Range':self.radar.acf_slant_range}
    ...
    def generate_time(self):
        ...
        Time['doy'] = np.array([[t.timetuple().tm_yday for t in ip] for ip in self.time])
        Time['dtime'] = np.array([[(t-dt.datetime(t.year,t.month,t.day,0,0,0)).total_seconds()/3600. for t in ip] for ip in self.time])

        _, mlt0 = self.iono.apex.convert(self.radar.site_lat, self.radar.site_lon, 'geo', 'mlt', height=self.radar.site_alt/1000., datetime=self.time[:,0])
        _, mlt1 = self.iono.apex.convert(self.radar.site_lat, self.radar.site_lon, 'geo', 'mlt', height=self.radar.site_alt/1000., datetime=self.time[:,1])
    ...
    def generate_geomag(self):
        # generate Geomag array
        # Reqires running IGRF to get all fields
        Geomag = {'Latitude':self.radar.lat, 'Longitude':self.radar.lon, 'Altitude':self.radar.alt}
        kvec = self.radar.kvec_all_gates()
    ...
    def generate_site(self):
        Site = {'Latitude':self.radar.site_lat, 'Longitude':self.radar.site_lon, 'Altitude':self.radar.site_alt, 'Code':0}
        mlat, mlon = self.iono.apex.geo2apex(self.radar.site_lat, self.radar.site_lon, height=self.radar.site_alt/1000.)
        _, mlt = self.iono.apex.convert(self.radar.site_lat, self.radar.site_lon, 'geo', 'mlt', height=self.radar.site_alt/1000., datetime=self.time[0,0])
        Site.update(MagneticLatitude=mlat, MagneticLongitude=mlon, MagneticLocalTimeMidnight=mlt)
        # self.Site.update(MagneticLatitude=0., MagneticLongitude=0., MagneticLocalTimeMidnight=0.)
    ...
    def create_summary_plots(self, plot_time=None, plot_beam=None, output_prefix='synthdata_summary_', alt_slices=[100000., 200000., 300000., 400000., 500000.], slice_xrng=[-500000., 500000., 10000.], slice_yrng=[-200000., 800000., 10000.], dens_colors={'vmin': 0., 'vmax':4.e11, 'cmap':'viridis'}, itemp_colors={'vmin':0., 'vmax':3000., 'cmap':'magma'}, etemp_colors={'vmin':0., 'vmax':5000., 'cmap':'inferno'}, vlos_colors={'vmin':-500., 'vmax':500., 'cmap':'coolwarm'}):
        ...
        #tidx = np.argmin(np.abs((plot_time-dt.datetime.utcfromtimestamp(0)).total_seconds()-self.Time['UnixTime'][:,0]))
        tidx = np.argmin(np.abs((plot_time-dt.datetime.utcfromtimestamp(0)).total_seconds()-self.utime[:,0]))

        ne0 = np.squeeze(self.iono.density(self.utime[tidx,0], glat, glon, galt))
        te0 = np.squeeze(self.iono.etemp(self.utime[tidx,0], glat, glon, galt))
        ti0 = np.squeeze(self.iono.itemp(self.utime[tidx,0], glat, glon, galt))
        ve = np.squeeze(self.iono.velocity(self.utime[tidx, 0], glat, glon, galt))
        ...
        plotting_params = [dict(synthdata=self.ne, param=ne0, cparam=dens_colors, label=r'Ne (m$^{-3}$)', title='Electron Density', output=output_prefix+'ne.png'),
                           dict(synthdata=self.ti, param=ti0, cparam=itemp_colors, label=r'Ti (K)', title='Ion Temperature', output=output_prefix+'ti.png'),
                           dict(synthdata=self.te, param=te0, cparam=etemp_colors, label=r'Te (K)', title='Electron Temperature', output=output_prefix+'te.png'),
                           dict(synthdata=self.vlos, param=[e,n], cparam=vlos_colors, label=r'Vlos (m/s)', title='Plasma Velocity', output=output_prefix+'vlos.png')]
        ...
        for p in plotting_params:
            ...
            ax.set_title('Beam Number: {:.0f} (az:{:.1f}, el:{:.1f})'.format(self.radar.beam_codes[bidx], self.radar.beam_azimuth[bidx], self.radar.beam_elevation[bidx]))
